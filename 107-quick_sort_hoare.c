...
int order_hoare(int *array, size_t size, int left, int right);
...
/**
 * order_hoare - Order a subset of an array of integers
...
void sort_hoare(int *array, size_t size, int left, int right)
{
	...
	if (right - left > 0)
	{
		part = order_hoare(array, size, left, right);
		sort_hoare(array, size, left, part - 1);
		sort_hoare(array, size, part, right);
	}
}
